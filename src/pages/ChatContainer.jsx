...
export default function ChatContainer() {
    ...
    const handleGlobalMessage = (data) => {
      setRecentChats((prev) => {
        ...
        const updatedChat = existingChat 
          ? { ...existingChat, lastMsg: data.text, time: messageTime, unreadCount: isNotActive ? (existingChat.unreadCount || 0) + 1 : 0 }
          : { id: senderId, name: data.senderName || "New Message", lastMsg: data.text, time: messageTime, unreadCount: isNotActive ? 1 : 0 };
  ...
  const handleMessageSent = (text) => {
    ...
    setRecentChats((prev) => {
      ...
      const updatedChat = { ...activeChat, lastMsg: text, id: chatId, time: formatSidebarTime(new Date()), unreadCount: 0 };
  ...
  const handleSelectChat = (chat) => {
    setActiveChat(chat);
    setRecentChats((prev) => prev.map((c) => String(c.id || c._id) === String(chat.id || chat._id) ? { ...c, unreadCount: 0 } : c));
  };
  ...
  return (
    <Container fluid className="vh-100 p-0 overflow-hidden" style={{ backgroundColor: "#fafaf9" }}>
      <Row className="h-100 g-0">
        <Col xs={12} md={4} lg={3} className={`border-end h-100 ${activeChat ? 'd-none d-md-block' : 'd-block'}`}>
          {loadingChats ? (
            <div className="d-flex h-100 align-items-center justify-content-center"><Spinner animation="border" variant="primary" /></div>
          ) : (
            <ChatSidebar contacts={recentChats} activeChat={activeChat} setActiveChat={handleSelectChat} onlineUsers={onlineUsers} socket={socket} />
          )}
        </Col>
        
        <Col 
          xs={12} md={8} lg={9} 
          className={`h-100 chat-window-mobile ${activeChat ? 'active' : ''} ${!activeChat ? 'd-none d-md-block' : 'd-block'}`}
        >
          <ChatWindow
            activeChat={activeChat ? { ...activeChat, status: onlineUsers.some(id => String(id) === String(activeChat.id || activeChat._id)) ? "Online" : "Offline" } : null}
            onBack={handleBack}
            socket={socket}
